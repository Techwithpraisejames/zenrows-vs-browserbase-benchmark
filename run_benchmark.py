...
def has_expected_content(target_name: str, html: str) -> bool:
    """Check whether the returned page contains the expected content for the target."""
    if not html:
        return False
        ...
        try:
            async with session.get(
                ...
                if api_status == 200:
                    ...
                    print(f"\nZENROWS API ERROR [{target['name']}]")
                    print(f"HTTP {api_status}: {error}\n")

        except Exception as exc:
            error = str(exc)
            print(f"\nZENROWS REQUEST ERROR [{target['name']}]")
...
# browserbase — fetch api with default settings
        ...
        try:
            async with session.post(
                ...
                if api_status == 200:
                    ...
                    error = (await resp.text())[:1000]
                    print(f"\nBROWSERBASE API ERROR [{target['name']}]")
                    print(f"HTTP {api_status}: {error}\n")

        except Exception as exc:
            error = str(exc)
            print(f"\nBROWSERBASE REQUEST ERROR [{target['name']}]")
            print(f"{type(exc).__name__}: {exc}\n")
        ...
        for target in targets:
            ...
            async def task(i, t=target):
                ...
                if i % 10 == 0 or i == 1:
                    ...
                    print(f"  [{i}/{REQUESTS_PER_TARGET}] {mark} {result['response_ms']}ms status={result['target_status_code']}")

                return result

            results = await asyncio.gather(*[task(i + 1) for i in range(REQUESTS_PER_TARGET)])
            successful = sum(1 for result in results if result["success"])
            print(f"  Done - {successful}/{REQUESTS_PER_TARGET} successful")
...
# output
RAW_FIELDS = [
    "tool", "target", "url", "api_status_code", "target_status_code",
    "response_ms", "has_title", "has_expected_content", "title", "content_type",
    "success", "error",
]
...
def print_table(results: list[dict], tool: str):
    ...
    print("=" * 85)
    print(f"  {'Target':<38} {'OK':>5} {'Total':>6} {'Rate':>8} {'Avg ms':>9}")
    print("-" * 85)

    for target, requests in by_target.items():
        successful = sum(1 for result in requests if result["success"])
        avg = round(statistics.mean(result["response_ms"] for result in requests))
        print(f"  {target[:38]:<38} {successful:>5} {len(requests):>6} {successful / len(requests) * 100:>7.1f}% {avg:>8}ms")
...
def main():
    ...
    for tool in tools:
        print("\n" + "=" * 85)
        print(f"  {tool.upper()} - {len(TARGETS)} targets x {REQUESTS_PER_TARGET} requests @ {RATE_PER_SECOND} req/s")
        print("=" * 85)
